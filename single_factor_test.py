...
4. 单因子回归测试模型思路：
    ...
    2. 以30天为一次训练单位，其中前 30 天的因子作为训练样本特征，
    3. 使用单变量线性模型进行训练。
    4. 回到当前时间点，使用前 30 天的因子数据作为预测样本特征，预测后 10 天的各股票平均收益率的大小。
5. 选股逻辑：
    将符合预测结果的股票按均等分配可用资金进行下单交易。持有 10 天之后，再次进行训练预测。
6. 交易逻辑：
...
def init(context):
    ...
    # 我们的目的就是使用前25天的因子数据作为样本，未来5天的股票收益率作为标签，进行回归。
    # 在前100天，可以取多组样本和标签，构成回归测试的数据
    context.N1 = 20   # 训练样本的时间跨度，代表训练的天数
    context.N2 = 5   # 标签的时间跨度,代表预测未来的天数
